...
extern uint8_t framebuffer[30 * 192];
...
uint32_t rom[] = {
  ...
  0xbbbbbbbb, 0xbbbbffff,
  0xbbbbbbbb, 0xffffbbbb,
  0xbbbbbbbb, 0xffffffff,
  0xbbbbffff, 0xbbbbbbbb,
  0xbbbbffff, 0xbbbbffff,
  0xbbbbffff, 0xffffbbbb,
  0xbbbbffff, 0xffffffff,
  0xffffbbbb, 0xbbbbbbbb,
  0xffffbbbb, 0xbbbbffff,
  0xffffbbbb, 0xffffbbbb,
  0xffffbbbb, 0xffffffff,
  0xffffffff, 0xbbbbbbbb,
  0xffffffff, 0xbbbbffff,
  0xffffffff, 0xffffbbbb,
  0xffffffff, 0xffffffff,
};
/*this rom is inverted*/
/*uint32_t rom[] = {
  0xffffffff, 0xffffffff,
  0xffffffff, 0xffffbbbb,
  0xffffffff, 0xbbbbffff,
...
LOCAL void slc_isr(void * unused) {
        ...
        if (slc_intr_status & SLC_RX_EOF_INT_ST) {
                ...
                else if( next->unused )
                {
                        ...
                        uint8_t *buffer_line = &framebuffer[30 * current_pixel_line];
                        ...
                        for( x = 0; x < 30; x++ )
                        {
                                ...
                                uint8_t b = buffer_line[x];
                                uint8_t h = ((b & 0xf0) >> 4) << 1;
                                uint8_t l = (b & 0x0f) << 1;
                                /* Use loop unrolling to improve performance */
                                buffer_pointer[pixel_column++] = rom[h]; //*character++;
                                buffer_pointer[pixel_column++] = rom[h+1]; //*character++;
                                buffer_pointer[pixel_column++] = rom[l]; //*character++;
                                buffer_pointer[pixel_column++] = rom[l+1]; //*character++;
                        }
